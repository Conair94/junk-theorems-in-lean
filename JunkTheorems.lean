...
lemma Set_instMeasurableSpace_generatedFrom {A : Type u} :
  ...
  rw [MeasurableSpace.measurableSpace_iSup_eq, id]
  ...
  · apply MeasurableSpace.generateFrom_mono
    ...
    apply Set.mem_setOf.1 at h
    cases h with
    | intro a h2 => unfold MeasurableSet
                    use a
                    rw [<- h2]
...
lemma GrpCat_Uncountable : Uncountable GrpCat := by
  constructor
  intro h
  apply countable_iff_exists_surjective.1 at h
  cases h with
  | intro f h2 => let X := Σ n : ℕ, (f n).carrier
                  let Y := Set X
                  ...
                  have h3 : ∀ n, Cardinal.mk (f n).carrier < Cardinal.mk Y := by
                    ...
                    have h4 : Cardinal.mk (f n).carrier ≤ Cardinal.mk X := by
                      apply @Cardinal.mk_le_of_injective _ _ (Sigma.mk (n : ℕ))
                      apply sigma_mk_injective
                    have h5 : Cardinal.mk X < Cardinal.mk (Set X) := by
                      rw [@Cardinal.mk_set X]
                      exact Cardinal.cantor (Cardinal.mk X)
...
theorem Exists_GrpCat_nonmeasurable : ¬MeasurableSet (@Exists GrpCat) := by
  ...
  | intro Y h3 => let h4 := h3.1
                  ...
                  have h6 : ∃ G, G ∈ .univ \ Y := by
                    ...
                    apply Set.not_countable_univ_iff.2 at h2
                    trivial
...
equal to the pair `⟨ℕ, 4294967295⟩`).
-/
